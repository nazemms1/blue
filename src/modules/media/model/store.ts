...
const MOCK_ITEMS: MediaItem[] = Array.from({ length: 12 }, (_, i) => ({
  id: generateId(),
  name: `Asset ${i + 1}`,
...
export function useMediaStore() {
  const [items, setItems] = useState<MediaItem[]>(MOCK_ITEMS)
  const [loading, setLoading] = useState(false)

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const fetchItems = useCallback(async (_filters?: MediaFilters) => {
    setLoading(true)
